...
public class ControllerToView {
	...
	public ModelAndView modelAndViewEx() {	// return 타입은 String이 아닌 ModelAndView 클래스로 작성한다.
		...
		for (int i = 1; i < 21; i++) {
			MemberDto memberDto = new MemberDto();
			memberDto.setMemberId("memberId" + i);
			memberDto.setMemberName("메가" + i);
			memberDto.setHp("010-1234-1234");
			memberDto.setMemberGender("M");
			memberDto.setEmail("mega@gmail.com");
			memberDto.setRegidence("서울특별시");
			memberList.add(memberDto);
		}
		
		mv.addObject("method" , "ModelAndView");	//addObject() 매서드에 전송할 데이터를 작성한다.
	...
	public String requestEx(HttpServletRequest request) {
		...
		for (int i = 11; i < 31; i++) {
			MemberDto memberDto = new MemberDto();
			memberDto.setMemberId("memberId" + i);
			memberDto.setMemberName("스터디" + i);
			memberDto.setHp("010-1234-1234");
			memberDto.setMemberGender("M");
			memberDto.setEmail("study@gmail.com");
			memberDto.setRegidence("서울특별시");
			memberList.add(memberDto);
		}
	 ...
	 *	# 예시 4) ResponseBody  
	 *	
	 *	- ResponseBody와 기능이 같으며 헤더와 상태 코드를 제외한 html 본문만 전송한다.//본문만보내서 한글적용이안됨
...
class RestControllerEx {
	...
	public String responseBodyEx() {
		...
		String data = "<script>";
				...
				data += "location.href='modelAndViewEx'";
				data +="</script>";
		return data;
